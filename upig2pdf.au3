...
#AutoIt3Wrapper_icon=upig2pdf.ico
#AutoIt3Wrapper_Res_Comment=upig2pdf by 31531640@qq.com http://17memo.com
#AutoIt3Wrapper_Res_Description=upig2pdf txtlrf
...
Global $g_inifileName = @ScriptDir & "\upig2pdf.ini"
...
;GUICtrlSetData($ctlEditParamTxt2lrf, 	IniRead($g_inifileName, $g_iniSection, "$ctlEditParamTxt2lrf", 		"--author=upig2pdf --left-margin=0 --right-margin=0 --top-margin=0 --bottom-margin=0 --override-css=""code {font-family: 'Swis721 BT'}"""))
...
Global $g_ToolName = 					IniRead($g_inifileName, $g_iniSection, "$g_ToolName", 			"pdfgen")
...
;for debug
If $g_bRelease==False Then _DebugSetup("upig2pdf")

;GUICtrlSetState(

;"upig2pdf工具用于将中文txt文件批量转换为lrf文件" & @CRLF & @CRLF & "请将需要转换的文件或文件夹拖进来" & @CRLF & "支持多个文件，文件夹一起拖" & @CRLF &"只支持遍历一级文件夹"&@CRLF&@CRLF &@CRLF &@CRLF &@CRLF &@CRLF & "注意:运行本软件先得安装Calibre(http://calibre.kovidgoyal.net)" & @CRLF & "说明:文件转换出错或出现乱码后可尝试修改GBK等参数" & @CRLF & "联系:有什么Bug欢迎反馈给我(31531640@qq.com)"
...
If $g_reg ==-1 Then
	RegWrite("HKEY_CLASSES_ROOT\txtfile\shell\A用&upig2pdf转换\command", "", "REG_SZ", @ScriptFullPath&" ""%1""")
	RegWrite("HKEY_CLASSES_ROOT\txtfile\shell\A用&upig2pdf转换\", "", "REG_SZ", "用&upig2pdf转换")
	RegWrite("HKEY_CLASSES_ROOT\Folder\shell\A用&upig2pdf转换\command", "", "REG_SZ", @ScriptFullPath&" ""%1""")
	RegWrite("HKEY_CLASSES_ROOT\Folder\shell\A用&upig2pdf转换\", "", "REG_SZ", "用&upig2pdf转换")
	$g_reg = 1
EndIf

If $g_reg ==0 Then
	RegDelete("HKEY_CLASSES_ROOT\txtfile\shell\A用&upig2pdf转换\")
	RegDelete("HKEY_CLASSES_ROOT\Folder\shell\A用&upig2pdf转换\")
EndIf
